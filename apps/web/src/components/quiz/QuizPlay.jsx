...
export default function QuizPlay({ questions, quizType, subject, period, onBack }) {
  ...
  const next = () => {
    if (current + 1 >= questions.length) {
      setFinished(true);
      // Essay quizzes give 5 XP per question for completion; multiple choice gives 10 XP per correct answer
      const xpGained = quizType === 'essay' ? questions.length * 5 : score * 10;
  ...
  if (finished) {
    const pct = quizType === 'essay' ? 100 : Math.round((score / questions.length) * 100);
    const xpGained = quizType === 'essay' ? questions.length * 5 : score * 10;
    return (
      <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
        <div className="bg-card border border-border rounded-2xl p-8 text-center">
          <div className="text-5xl font-bold text-primary mb-2">{pct}%</div>
          <p className="text-foreground font-semibold text-lg">{quizType === 'essay' ? `${questions.length}/${questions.length} questões revisadas` : `${score}/${questions.length} corretas`}</p>
          <p className="text-muted-foreground mt-1 text-sm">+{xpGained} XP ganhos!</p>
